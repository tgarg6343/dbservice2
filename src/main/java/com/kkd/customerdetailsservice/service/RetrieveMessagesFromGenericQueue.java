...
import com.kkd.customerdetailsservice.CustomerDetailsServiceApplication;

//a service class that retrieves the messages from the rabbitmq server queue
@Service
public class RetrieveMessagesFromGenericQueue {
	// to show the logs
	private static final Logger log = LoggerFactory.getLogger(CustomerDetailsServiceApplication.class);

	// listing to generic queue
	@RabbitListener(queues = CustomerDetailsServiceApplication.QUEUE_GENERIC_NAME)
